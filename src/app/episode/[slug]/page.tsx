...
// ---- MAIN EPISODE PAGE (CLIENT COMPONENT) ----
export default function EpisodePage() {
  ...
  useEffect(() => {
    async function fetchEpisode() {
      try {
        const res = await fetch('http://localhost:3000/api/episodes', { cache: 'no-store' });
        if (!res.ok) {
          setError(true);
          setLoading(false);
          return;
        ...
        const foundEpisode = episodes.find(
          (ep) => slugifyTitle(ep.title) === params.slug
        );
        if (!foundEpisode) {
          setError(true);
        } else {
  ...
  if (error || !episode) {
    return <div className="text-center mt-20">Episode Not Found</div>;
  }
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8 py-8 text-[#28282B]">
        <div className="mt-16 sm:mt-24 mb-16">
          ...
          <div className="mb-4 flex items-center flex-wrap gap-2">
            {episode.type && <span className="text-green-600 font-medium">{episode.type}</span>}
          ...
          {episode.categories && episode.categories.length > 0 && (
            <div className="mb-8 flex flex-wrap gap-2">
              {episode.categories.map((category) => (
                <span key={category} className="text-[#28282B] bg-gray-100 px-2 py-1 rounded">
                  {category}
                </span>
              ))}
            </div>
...
// ---- METADATA GENERATION (Cannot be Client Component) ----
// Metadata should still be handled in a server component.
// To achieve this, you can create a separate server component or handle metadata differently.
