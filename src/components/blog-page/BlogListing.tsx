...
const BlogListing = () => {
  ...
  useEffect(() => {
    const fetchBlogPosts = async () => {
      ...
      setError(null);
      try {
        const response = await fetch('/api/blog'); // Fetch all articles
        if (!response.ok) {
  ...
  // Sort blog posts based on sortOrder
  ...
  return (
    <div className="w-full max-w-7xl mx-auto px-4 md:px-16 py-8 md:py-16">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative flex-1" ref={dropdownRef}>
            ...
            >
              ...
              <svg
                className={`w-4 h-4 transition-transform text-black ml-2 ${
              ...
              <div className="absolute right-0 mt-2 w-full md:w-48 bg-white border border-gray-200 rounded-lg shadow-lg z-10">
                <button
                  ...
                  className={`w-full px-4 py-2 text-left hover:bg-gray-50 text-black ${
                    sortOrder === 'newest' ? 'font-bold' : 'font-normal'
                ...
                <button
                  ...
                  className={`w-full px-4 py-2 text-left hover:bg-gray-50 text-[#28282B] ${
                    sortOrder === 'oldest' ? 'font-bold' : 'font-normal'
                  } text-sm md:text-base`}
                >
                  Date: Oldest first
                </button>
      ...
      <div className="w-full">
        ...
        {!loading && !error && sortedBlogPosts.length === 0 && (
          <p className="text-center text-gray-500">No blog posts found.</p>
        )}
        {!loading &&
          !error &&
          sortedBlogPosts.map((blog) => (
            <div key={blog.PostID} className="group border-b border-gray-200">
      ...
      {/* Pagination */}
      {/* TODO: Implement pagination based on your requirements */}
